...
Pregunta: ¿Es posible predecir si un paciente tiene enfermedad cardíaca
a partir de sus indicadores clínicos?
...
# Imputación con mediana (igual que en el notebook)
...
df['sexo_str'] = df['sex'].map({0.0: 'Femenino', 1.0: 'Masculino'})
...
# Datos del mapa (distritos de Panamá)
gdf = gpd.read_file(os.path.join(BASE_DIR, 'panama_distritos.geojson'))
poblacion = pd.read_csv(os.path.join(BASE_DIR, 'poblacion_distritos_panama.csv'))
gdf = gdf.merge(poblacion, left_on='shapeName', right_on='distrito', how='left')
...
app.layout = html.Div(style={'backgroundColor': COLOR_BG, 'padding': '20px',
    ...
    html.Div(style={'display': 'grid', 'gridTemplateColumns': '1fr 1fr', 'gap': '20px'}, children=[
        ...
        html.Div(style=card_style, children=[
            html.H4('2. Edad vs Frec. Cardíaca Máx.', style={'color': COLOR_PRIMARIO}),
    ...
    # Fila 3: Predictor de colesterol
    html.Div(style=card_style, children=[
        html.H3('Predictor de Colesterol (Modelo de Regresión)',
                style={'color': COLOR_PRIMARIO}),
        html.P('Ingrese los datos del paciente y el modelo Random Forest Regressor '
               'predecirá su nivel de colesterol estimado (mg/dl).',
               style={'fontSize': '13px', 'color': '#666'}),

        html.Div(style={'display': 'grid', 'gridTemplateColumns': '1fr 1fr 1fr',
                        'gap': '15px'}, children=[
            html.Div([html.Label('Edad:'),
                      dcc.Input(id='in-age', type='number', value=54, min=20, max=100,
                                style={'width': '100%'})]),
            html.Div([html.Label('Sexo (0=F, 1=M):'),
                      dcc.Input(id='in-sex', type='number', value=1, min=0, max=1,
                                style={'width': '100%'})]),
            html.Div([html.Label('Tipo dolor pecho (1-4):'),
                      dcc.Input(id='in-cp', type='number', value=3, min=1, max=4,
                                style={'width': '100%'})]),
            html.Div([html.Label('Presión arterial:'),
                      dcc.Input(id='in-trestbps', type='number', value=130, min=80, max=220,
                                style={'width': '100%'})]),
            html.Div([html.Label('Azúcar >120 (0/1):'),
                      dcc.Input(id='in-fbs', type='number', value=0, min=0, max=1,
                                style={'width': '100%'})]),
            html.Div([html.Label('ECG reposo (0-2):'),
                      dcc.Input(id='in-restecg', type='number', value=1, min=0, max=2,
                                style={'width': '100%'})]),
            html.Div([html.Label('Frec. cardíaca máx:'),
                      dcc.Input(id='in-thalach', type='number', value=150, min=70, max=220,
                                style={'width': '100%'})]),
            html.Div([html.Label('Angina ejercicio (0/1):'),
                      dcc.Input(id='in-exang', type='number', value=0, min=0, max=1,
                                style={'width': '100%'})]),
            html.Div([html.Label('Depresión ST:'),
                      dcc.Input(id='in-oldpeak', type='number', value=1.0, step=0.1,
                                style={'width': '100%'})]),
            html.Div([html.Label('Pendiente ST (1-3):'),
                      dcc.Input(id='in-slope', type='number', value=2, min=1, max=3,
                                style={'width': '100%'})]),
            html.Div([html.Label('Vasos coloreados (0-3):'),
                      dcc.Input(id='in-ca', type='number', value=0, min=0, max=3,
                                style={'width': '100%'})]),
            html.Div([html.Label('Talasemia (3, 6, 7):'),
                      dcc.Input(id='in-thal', type='number', value=3, min=3, max=7,
                                style={'width': '100%'})]),
        ]),
        ...
        html.Button('Predecir Colesterol', id='btn-predecir', n_clicks=0,
                    style={'backgroundColor': COLOR_SEC, 'color': 'white',
                           ...
                           'fontSize': '15px'}),
        html.Br(), html.Br(),
        html.Div(id='resultado-prediccion', style={'fontSize': '20px',
                                                    'fontWeight': 'bold',
                                                    'color': COLOR_PRIMARIO})
    ]),

    # Fila 4: Mapa de Panamá + resumen
    html.Div(style=card_style, children=[
        html.H3('Mapa Interactivo - Población por Distrito (Panamá)',
                style={'color': COLOR_PRIMARIO}),
        html.P('Datos aproximados basados en cifras del INEC. '
               'Fuente boundaries: geoBoundaries.org (76 distritos).',
               style={'fontSize': '13px', 'color': '#666'}),

        html.Div(style={'display': 'grid', 'gridTemplateColumns': '2fr 1fr',
                        'gap': '20px'}, children=[
            html.Div([dcc.Graph(id='mapa-panama')]),
            html.Div([dcc.Graph(id='top-distritos')])
        ])
    ]),

    # Footer
    html.Div(style={'textAlign': 'center', 'padding': '15px', 'color': '#888',
                    'fontSize': '12px'}, children=[
        html.P('Proyecto Final 2026 - Análisis de Datos y Toma de Decisiones en Computación'),
        html.P('Facilitador: José Carlos Rangel Ortiz, PhD.')
    ])
])
...
def filtrar_df(rango_edad, sexo):
    """Filtra el dataset por los controles."""
...
def actualizar_graficas(rango_edad, sexo):
    ...
    if len(d) == 0:
        fig_vacia = go.Figure().add_annotation(
            text='No hay datos con estos filtros', showarrow=False)
        return fig_vacia, fig_vacia, fig_vacia, fig_vacia

    # 1. Gráfica de torta: distribución del diagnóstico
    conteo = d['diagnostico'].value_counts()
    fig1 = px.pie(values=conteo.values, names=conteo.index,
                  color=conteo.index,
                  color_discrete_map={'Sin Enfermedad': '#2ecc71',
    ...
    fig2 = px.scatter(d, x='age', y='thalach', color='diagnostico',
                      color_discrete_map={'Sin Enfermedad': '#2ecc71',
                                          'Con Enfermedad': '#e74c3c'},
                      labels={'age': 'Edad', 'thalach': 'Frec. Cardíaca Máx.'},
                      opacity=0.7)
    fig2.update_layout(margin=dict(l=20, r=20, t=20, b=20), height=300,
                       legend=dict(orientation='h', y=-0.2))

    # 3. Boxplot colesterol por diagnóstico
    ...
    # 4. Barras cp por diagnóstico
    cp_labels = {1.0: 'Angina típica', 2.0: 'Angina atípica',
                 3.0: 'No angina', 4.0: 'Asintomático'}
    ...
    fig4 = px.bar(conteo_cp, x='cp_str', y='cantidad', color='diagnostico',
                  color_discrete_map={'Sin Enfermedad': '#2ecc71',
                                      'Con Enfermedad': '#e74c3c'},
                  barmode='group', labels={'cp_str': 'Tipo de dolor',
                                            'cantidad': 'Cantidad'})
...
@app.callback(
    ...
    [Input('in-age', 'value'), Input('in-sex', 'value'), Input('in-cp', 'value'),
     Input('in-trestbps', 'value'), Input('in-fbs', 'value'),
     Input('in-restecg', 'value'), Input('in-thalach', 'value'),
     Input('in-exang', 'value'), Input('in-oldpeak', 'value'),
...
def predecir_colesterol(n_clicks, age, sex, cp, trestbps, fbs, restecg,
    ...
    try:
        ...
        # Reordenar columnas igual que el modelo espera
        instancia = instancia[columnas_modelo]
...
@app.callback(
    [Output('mapa-panama', 'figure'),
     Output('top-distritos', 'figure')],
    Input('slider-edad', 'value')  # solo para activar el callback
)
def actualizar_mapa(_):
    ...
    top10 = gdf.nlargest(10, 'poblacion')[['shapeName', 'poblacion']]
    fig_top = px.bar(top10.sort_values('poblacion'),
                     x='poblacion', y='shapeName', orientation='h',
                     color='poblacion', color_continuous_scale='Viridis',
                     labels={'poblacion': 'Población', 'shapeName': ''},
